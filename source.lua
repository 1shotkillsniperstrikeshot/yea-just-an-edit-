...
function Library:create(options)
	...
	if readfile and writefile and isfile then
		if not isfile("AmogusSettings.json") then
			writefile("AmogusSettings.json", HTTPService:JSONEncode(settings))
		end
		settings = HTTPService:JSONDecode(readfile("AmogusSettings.json"))
	...
	options = self:set_defaults({
		...
		Link = "The rock and amogus and hank and deimos and hank secret base"
	}, options)

	if getgenv and getgenv().AmogusUI then
		getgenv():AmogusUI()
		getgenv().AmogusUI = nil
	end
